...
Combining the sound (eg combines first video with the third audio of second):

```
ffmpeg -i 'input0.mkv' -i 'input1.mkv' -c copy -map 0:0 -map 1:3 -shortest 'output.mkv'
...
```
ffmpeg -i 'input.mkv' -i 'input.srt' -map 0 -map 1 -c copy -metadata:s:s:0 language=eng output.mkv
```

Compressing to h.265

```
ffmpeg -i input.mkv -vf scale=1920:1080 -vcodec libx265 -crf 28 output.mkv
```
